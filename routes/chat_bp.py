...
from utils.chat_utils import query_assistant
...
chat_bp = Blueprint('chat_bp', __name__)     # instanciar admin_bp desde clase Blueprint para crear las rutas.
bcrypt = Bcrypt()
jwt = JWTManager()
...
def authorize():
    if request.method == 'OPTIONS':
        return
    if request.path in ['/test_clasifica_chat_bp']:
        return
...
@chat_bp.route('/test_clasifica_chat_bp', methods=['GET'])
def test():
    logger.info("Chat moes bp rutas funcionando ok segun test.")
    return jsonify({'message': 'test bien sucedido','status':"Si lees esto, chat rutas funcionan bien..."}),200
...
@chat_bp.route("/close_chat", methods=["POST"])
def close_chat():
    """
