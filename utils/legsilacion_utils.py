...
def ask_to_openai(question):
    try:
        prompt = (
            "Según la Ley General de Instituciones y Procedimientos Electorales de México (disponible en mexico.justia.com),\n"
            f"{question}\n"
            "Por favor, indícame el artículo correspondiente exacto.\n\n"
            "Prioriza sitios web oficiales o confiables, como:\n"
            "INE.mx (Instituto Nacional Electoral).\n"
            "mexico.justia.com (base legal confiable).\n"
            "Sitios del gobierno mexicano (con extensiones .gob.mx)."
        )
        ...
        logger.error(f"Error al procesar el mensaje {question}: {e}")
