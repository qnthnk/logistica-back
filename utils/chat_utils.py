...
ASSISTANT_ID = "asst_xPKdzudwxiLfYSGeLc02qY5g"
...
def query_assistant(prompt: str, thread_id: Optional[str] = None) -> Tuple[str, str]:
    ...
    instruction_prefix = (
        "Se te va a presentar una pregunta relacionada con el mejor recorrido a partir de diferentes direcciones. "
        "Tu tarea es responder exclusivamente sobre esos temas. Si el usuario realiza una consulta "
        "que no esté vinculada a lo mencionado, respondé que solo estás habilitado para asistir en temas relacionados con los mejores recorridos de direcciones que te piden y que ya tenés.\n\n"
        "Además, si el usuario pregunta quién sos, cómo funcionás o si sos un experto, aclarales que sos un asistente creado por Quanthink "
        "para asistir en consultas vinculadas al contenido de mejores rutas.\n\n"
